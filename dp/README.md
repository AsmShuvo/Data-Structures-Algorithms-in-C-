...
- **Dynamic Programming (DP):** An optimization over plain recursion. It stores the results of subproblems so that we don't re-calculate them. This is useful when the problem has *overlapping subproblems* and *optimal substructure*.
...
1.  **Top-Down (Memoization):**
    ...
    -   *Why:* Natural to write if you already have the recursive solution.
2.  **Bottom-Up (Tabulation):**
    -   Start from the smallest subproblems (base cases) and build up to the main problem.
    -   Fill a table iteratively.
    -   *Why:* Usually saves stack space (no recursion overhead) and can sometimes be optimized further (space optimization).
...
3.  [Knapsack Problem (GeeksForGeeks/Spoj)](https://www.spoj.com/problems/KNAPSACK/)
